<template>
  <b-navbar toggleable="md" fixed="top" type="dark" variant="dark">
    ...
    <b-navbar-brand href="https://nexuspad.com/about.html" v-if="!isLoggedIn">
      <!-- <img
          style="width: 26px"
          :class="{ imageRotateHorizontal: loadingIcon }"
          src="https://nexuspad.com/images/np-logo.png"
        /> -->
      <span
        style="
          margin-left: 1em;
    ...
    <b-collapse is-nav id="topnav_collapse" v-if="isLoggedIn === true">
      <b-navbar-nav>
        <b-nav-item
          ...
          :to="m.link"
          v-bind:key="m.id"
          :class="{ highlight: activeModule == m.id }"
        ...
        <b-nav-form @submit="search">
          <input
            ...
            v-on:keyup.delete="clearSearch($event)"
          />
          <b-button class="my-2 my-sm-0" v-on:click="search($event)">{{
            npContent("search")
          }}</b-button>
        </b-nav-form>
      </b-navbar-nav>
...
export default {
  ...
  mounted() {
    ...
    EventManager.subscribe(
      AppEvent.ACCOUNT_MODULE_SETTINGS_UPDATE,
      this.setModules
    );
        ...
        .catch(function (error) {
          ...
          if (error instanceof NPError) {
            if (
              ...
              componentSelf.$router.currentRoute.meta.requiresAuth === true
            ) {
